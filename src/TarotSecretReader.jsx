...
function detectQuestionType(question) {
  ...
  if (/รัก|แฟน|เขา|ความสัมพันธ์|คนคุย|แต่งงาน/.test(q)) return "ความรัก/ความสัมพันธ์";
  if (/งาน|อาชีพ|สัมภาษณ์|หัวหน้า|ธุรกิจ/.test(q)) return "งาน/อาชีพ";
  if (/เงิน|รายได้|หนี้|ลงทุน|การเงิน/.test(q)) return "การเงิน";
  if (/ควร|เลือก|ตัดสินใจ|ทางไหนดี/.test(q)) return "การตัดสินใจ";
  if (/จะ.*ไหม|ไหม$|หรือไม่/.test(q)) return "คำถามปลายปิด";
...
function makeLocalReading({ question, desiredInfo, context, cards }) {
  ...
  const keyIndex = c3.sub === "Major Arcana" ? 2 : (c2.sub === "Major Arcana" ? 1 : 1);
  const key = cards[keyIndex];
  const keyMeaning = meanings[keyIndex];
  const status = c => c.reversed ? "กลับหัว" : "ตั้งตรง";
  const clarity = majorCount >= 2 ? "HIGH" : (revCount >= 2 ? "MEDIUM" : "MEDIUM");
  const desired = desiredInfo.trim() ? ` โดยเฉพาะสิ่งที่คุณอยากรู้ว่า “${desiredInfo.trim()}”` : "";
  const ctx = context.trim() ? ` บริบทที่คุณให้มา (“${context.trim()}”) ทำให้ควรอ่านไพ่โดยยึดสิ่งที่เกิดขึ้นจริงเป็นหลัก` : "";

  let answer = "คำตอบยังไม่ใช่คำว่าใช่หรือไม่แบบตายตัว แต่ไพ่ชี้ว่าผลจะขึ้นกับวิธีที่คุณจัดการสถานการณ์ในช่วงนี้";
  if (type === "คำถามปลายปิด") {
    answer = revCount >= 2
      ? "คำตอบเอนเอียงไปทาง **CONDITIONAL** — มีโอกาส แต่ยังมีเงื่อนไขหรือจุดติดขัดที่ต้องแก้ก่อน"
      : "คำตอบเอนเอียงไปทาง **YES, BUT...** — แนวโน้มไปได้ แต่ต้องเดินต่ออย่างมีสติและไม่เร่งผลลัพธ์";
  } else if (type === "ความรัก/ความสัมพันธ์") {
    answer = "ไพ่ชี้ว่าความสัมพันธ์นี้ต้องการความชัดเจนมากกว่าการเดาใจ สิ่งสำคัญคือดูการกระทำจริงและคุยสิ่งที่ค้างให้ตรงกัน";
  } else if (type === "งาน/อาชีพ") {
    answer = "ไพ่ชี้ว่าคุณกำลังอยู่ในช่วงจัดทิศทางใหม่ งานจะขยับได้ดีเมื่อโฟกัสสิ่งที่ควบคุมได้และวางขั้นตอนให้ชัด";
  } else if (type === "การเงิน") {
    answer = "ไพ่เน้นการจัดการทรัพยากรและความเสี่ยงแบบมีข้อมูล มากกว่าการหวังผลจากจังหวะเพียงอย่างเดียว";
  } else if (type === "การตัดสินใจ") {
    answer = "ไพ่ไม่ได้เลือกแทนคุณ แต่ชี้ว่าทางที่ดีคือทางที่ลดความคลุมเครือ เพิ่มความสอดคล้องกับคุณค่า และทำได้จริงในระยะยาว";
  }

  const hidden = revCount
    ? "สิ่งที่อาจมองข้ามคือปัญหาไม่ได้อยู่ที่ “ไม่มีทางไป” แต่อยู่ที่พลังบางส่วนยังติดขัด คุณอาจต้องหยุดแก้ที่ปลายเหตุแล้วกลับไปดูเงื่อนไขที่ทำให้เรื่องนี้วนซ้ำ"
    : "สิ่งที่อาจมองข้ามคือไพ่ทั้งชุดไม่ได้เรียกร้องให้คุณรู้คำตอบทั้งหมดทันที แต่มันชวนให้ใช้สิ่งที่รู้อยู่แล้วให้เป็นระบบ แล้วดูผลจากการลงมือจริงทีละขั้น";

  const caution = revCount >= 2
    ? "ระวังการรีบสรุปจากความกลัวหรือความไม่ชัดเจน เพราะไพ่กลับหัวหลายใบมักสะท้อนพลังที่ยังไม่นิ่งมากกว่าคำตัดสินสุดท้าย"
    : "ระวังการมองสัญญาณบวกแล้วข้ามรายละเอียดเล็ก ๆ เพราะความคืบหน้าจะยั่งยืนเมื่อมีทั้งความหวังและการตรวจสอบข้อเท็จจริง";

  return `# 🐾 คำตอบหลัก\n${answer}${desired}\n\n# 🃏 เรื่องราวของไพ่\n**ใบที่ 1 — ${c1.name} (${status(c1)})** เปิดเรื่องด้วย ${meanings[0]} จากนั้น **ใบที่ 2 — ${c2.name} (${status(c2)})** แสดงกลไกตรงกลางว่า ${meanings[1]} และ **ใบที่ 3 — ${c3.name} (${status(c3)})** พาเรื่องไปสู่ ${meanings[2]} ดังนั้นภาพรวมไม่ใช่ไพ่สามใบที่แยกกัน แต่เป็นเส้นทางจาก “สิ่งที่กำลังเกิด” → “สิ่งที่ขับเคลื่อน” → “ทิศทางถัดไป”${ctx}\n\n# 🔑 ไพ่กุญแจ\n**${key.name} (${status(key)})** เป็นไพ่กุญแจ เพราะอยู่ในตำแหน่งที่อธิบายแกนของเรื่องได้ชัดที่สุดในชุดนี้ ความหมายหลักคือ ${keyMeaning}\n\n# 🌙 สิ่งที่กำลังเกิดขึ้น\nคำถามนี้อยู่ในกลุ่ม **${type}** และไพ่กำลังชี้ให้คุณดูความสัมพันธ์ระหว่างความรู้สึกกับสิ่งที่ทำได้จริง ถ้าคุณแยก “สิ่งที่รู้จริง” ออกจาก “สิ่งที่กำลังกังวลหรือคาดหวัง” ภาพจะชัดขึ้นมาก\n\n# ✨ ความลับที่เปิดเผย\n${hidden} นี่ไม่ใช่ความลับเกี่ยวกับคนอื่น แต่เป็นรูปแบบของสถานการณ์ที่ไพ่กำลังสะท้อนกลับมาให้เห็น\n\n# ⚠️ สิ่งที่ต้องระวัง\n${caution}\n\n# 🐱 สิ่งที่ควรทำต่อ\n**DO:** เลือกหนึ่งเรื่องที่คุณควบคุมได้และลงมือให้ชัดในช่วงสั้น ๆ ต่อจากนี้\n\n**DON'T:** อย่าตัดสินอนาคตทั้งหมดจากอารมณ์หรือเหตุการณ์เพียงครั้งเดียว\n\n**KEY MOVE:** ใช้ข้อเท็จจริง + การสื่อสารตรงไปตรงมา + การดูผลจากการกระทำ เป็นเข็มทิศหลัก\n\n# 📊 ความชัดของคำตอบ\n**${clarity}** — ไพ่ให้ธีมร่วมกันพออ่านเป็นเรื่องเดียวได้ แต่ยังควรใช้เป็นมุมมองประกอบ ไม่ใช่ข้อเท็จจริงที่กำหนดอนาคต\n\n# 💫 บทสรุป\nตอนนี้เรื่องกำลังขยับจาก **${c1.name}** ผ่านกลไกของ **${c2.name}** ไปสู่ทิศทางของ **${c3.name}** จุดสำคัญคืออย่าพยายามควบคุมสิ่งที่ยังไม่รู้ทั้งหมด แต่ให้จัดการส่วนที่อยู่ในมือคุณให้ดี แล้วใช้ผลที่เกิดขึ้นจริงเป็นข้อมูลสำหรับก้าวถัดไป`;
}
