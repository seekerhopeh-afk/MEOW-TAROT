...
function cardLocalMeaning(card) {
  if (card.sub === "Major Arcana") {
    const pair = MAJOR_LOCAL[card.name] || ["พลังสำคัญที่กำลังเปลี่ยนมุมมองของเรื่องนี้", "พลังสำคัญที่ยังติดขัดหรือแสดงออกไม่เต็มที่"];
    return card.reversed ? pair[1] : pair[0];
  }
  ...
  if (!m) return "ประเด็นสำคัญที่ควรมองร่วมกับไพ่ใบอื่น";
  const [, rank, suit] = m;
  const base = `${RANK_LOCAL[rank]} ในเรื่อง${SUIT_LOCAL[suit]}`;
  return card.reversed ? `${base} แต่พลังนี้กำลังติดขัด ล่าช้า หรือแสดงออกในด้านที่ต้องทบทวน` : base;
}

function detectQuestionType(question) {
  const q = question.toLowerCase();
  if (/รัก|แฟน|เขา|เธอ|ความสัมพันธ์|คนคุย|แต่งงาน|รีเทิร์น|กลับมา|คิดถึง/.test(q)) return "ความรัก/ความสัมพันธ์";
  if (/งาน|อาชีพ|สัมภาษณ์|หัวหน้า|เจ้านาย|ธุรกิจ|โปรเจกต์|สมัครงาน|ย้ายงาน/.test(q)) return "งาน/อาชีพ";
  if (/เงิน|รายได้|หนี้|ลงทุน|การเงิน|กำไร|ขาย|ซื้อ|ค่าใช้จ่าย/.test(q)) return "การเงิน";
  if (/ควร|เลือก|ตัดสินใจ|ทางไหนดี|เอาแบบไหนดี|ไปต่อ|พอแค่นี้/.test(q)) return "การตัดสินใจ";
  if (/จะ.*ไหม|ไหม$|หรือไม่|มีโอกาสไหม|ได้ไหม/.test(q)) return "คำถามปลายปิด";
  return "ภาพรวมและแนวโน้ม";
}
...
function relationshipSentence(c1, c2, c3) {
  ...
  if (major >= 2) return "ชุดนี้มี Major Arcana เด่น จึงเหมือนกำลังพูดถึงบทเรียนหรือจุดเปลี่ยนที่มีน้ำหนักมากกว่ารายละเอียดรายวัน";
  if (rev >= 2) return "ไพ่กลับหัวหลายใบทำให้เรื่องนี้ดูเหมือนพลังยังไหลไม่เต็มที่ จึงควรอ่านเป็นจังหวะที่ต้องแก้เงื่อนไขหรือทบทวนวิธีเดิม";
  return "พลังของไพ่ทั้งสามค่อนข้างต่อเนื่องกัน จึงอ่านเป็นเรื่องเดียวได้ชัดกว่าการแยกความหมายทีละใบ";
}

function hiddenTruthFor(type, cards) {
  ...
  if (type === "งาน/อาชีพ") return "สิ่งที่ซ่อนอยู่หลังความกังวลอาจไม่ใช่ ‘ทำไม่ได้’ แต่เป็นการใช้แรงกับสิ่งที่ไม่ใช่คอขวดของปัญหา หากหาได้ว่าจุดไหนขยับแล้วส่วนอื่นจะตามมา คุณจะประหยัดแรงได้มาก";
  if (type === "การเงิน") return "สิ่งที่อาจมองข้ามคือความสบายใจทางการเงินไม่ได้มาจากรายรับอย่างเดียว แต่เกิดจากการรู้ขอบเขตของตัวเองชัดว่ารับความเสี่ยงได้แค่ไหนและต้องสำรองอะไรไว้";
  if (type === "การตัดสินใจ") return "สิ่งที่อาจมองข้ามคือบางครั้งความลังเลไม่ได้เกิดจากไม่มีคำตอบ แต่เกิดจากแต่ละทางเลือกมีราคาที่ต้องจ่าย ไพ่กำลังชวนให้คุณเลือก ‘ราคาที่รับได้’ มากกว่าตามหาทางที่ไม่มีข้อเสียเลย";
  return revCount
...
function makeLocalReading({ question, desiredInfo, context, cards }) {
  const [c1, c2, c3] = cards;
  const meanings = cards.map(cardLocalMeaning);
  const type = detectQuestionType(question);
  const topic = TOPIC_COPY[type] || TOPIC_COPY["ภาพรวมและแนวโน้ม"];
  const revCount = cards.filter(c => c.reversed).length;
  const majorCount = cards.filter(c => c.sub === "Major Arcana").length;
  const keyIndex = c2.sub === "Major Arcana" ? 1 : (c3.sub === "Major Arcana" ? 2 : 1);
  const key = cards[keyIndex];
  const keyMeaning = meanings[keyIndex];
  const status = c => c.reversed ? "กลับหัว" : "ตั้งตรง";
  const clarity = majorCount >= 2 ? "HIGH" : (revCount >= 2 ? "MEDIUM" : "MEDIUM–HIGH");
  const desired = desiredInfo.trim() ? ` โดยเฉพาะประเด็นที่คุณอยากรู้ว่า “${desiredInfo.trim()}”` : "";
  const ctx = context.trim() ? ` เมื่อเทียบกับบริบทที่คุณให้ไว้ (“${context.trim()}”) ควรยึดเหตุการณ์จริงเป็นตัวตรวจสอบคำอ่านนี้ด้วย` : "";

  let answer = topic.answer;
  if (type === "คำถามปลายปิด") {
    answer = revCount >= 2
      ? "คำตอบเอนเอียงไปทาง **CONDITIONAL / ยังไม่ลงตัว** — โอกาสไม่ได้ปิด แต่มีเงื่อนไขหรือจุดติดขัดที่ต้องคลี่คลายก่อน"
      : "คำตอบเอนเอียงไปทาง **YES, BUT...** — มีแนวโน้มไปต่อได้ แต่ผลขึ้นกับการจัดการรายละเอียดและการตัดสินใจระหว่างทาง";
  }

  const hidden = hiddenTruthFor(type, cards);
  const caution = revCount >= 2
    ? "ระวังการรีบสรุปจากความกลัวหรือความไม่ชัดเจน เพราะไพ่กลับหัวหลายใบสะท้อนพลังที่ยังไม่นิ่งมากกว่าคำตัดสินสุดท้าย"
    : "ระวังการเห็นสัญญาณที่เข้ากับสิ่งที่อยากเชื่อ แล้วมองข้ามรายละเอียดที่ไม่เข้ากัน คำตอบที่ดีควรผ่านการตรวจสอบกับสิ่งที่เกิดขึ้นจริง";
  const relation = relationshipSentence(c1, c2, c3);

  return `# 🐾 คำตอบตรงคำถาม\n${answer}${desired}\n\n# 🃏 ไพ่ทั้ง 3 ใบกำลังเล่าอะไร\n**${c1.name} (${status(c1)})** เปิดเรื่องด้วย ${meanings[0]} แล้ว **${c2.name} (${status(c2)})** เข้ามาเป็นจุดที่อธิบายว่าเหตุใดเรื่องจึงขยับแบบนี้ ผ่านพลังของ ${meanings[1]} ก่อนที่ **${c3.name} (${status(c3)})** จะพาไปสู่ทิศทางของ ${meanings[2]} ${relation}${ctx}\n\n# 🔑 ไพ่กุญแจ\n**${key.name} (${status(key)})** เป็นหัวใจของคำอ่านครั้งนี้ เพราะมันอยู่ตรงจุดที่เชื่อม “สิ่งที่เกิดขึ้น” กับ “สิ่งที่จะพาเรื่องไปต่อ” ความหมายของมันคือ ${keyMeaning}\n\n# 🌙 สิ่งที่กำลังเกิดขึ้น\n${topic.current} ลองใช้ไพ่ชุดนี้เป็นกระจกสะท้อนว่าอะไรคือข้อเท็จจริง อะไรคือความหวัง และอะไรคือความกลัวที่กำลังปะปนกันอยู่\n\n# ✨ สิ่งที่ไพ่อยากกระซิบ\n${hidden}\n\n# ⚠️ สิ่งที่ต้องระวัง\n${caution}\n\n# 🐱 สิ่งที่ควรทำต่อ\n**DO:** ${topic.doText}\n\n**DON'T:** ${topic.dontText}\n\n**KEY MOVE:** ${topic.keyMove}\n\n# 📊 ความชัดของคำตอบ\n**${clarity}** — ไพ่ทั้งสามมีธีมที่เชื่อมกันพอให้เห็นทิศทาง แต่ควรใช้เป็นมุมมองประกอบการตัดสินใจ ไม่ใช่ข้อเท็จจริงที่กำหนดอนาคต\n\n# 💫 บทสรุป\nเรื่องนี้เริ่มจากพลังของ **${c1.name}** ผ่านจุดเปลี่ยนของ **${c2.name}** แล้วกำลังเคลื่อนไปทาง **${c3.name}** คำสำคัญไม่ใช่ “ต้องเกิดอะไรแน่” แต่คือ “ตอนนี้คุณเห็นอะไรชัดขึ้น และจะใช้ความชัดนั้นทำอะไรต่อ” หากคุณเลือกการกระทำที่สอดคล้องกับข้อมูลจริง คำตอบจะค่อย ๆ ชัดขึ้นจากชีวิตจริงเอง`;
}
